...
export function BlockListMutator(blockData: BlockDefinition, id: string) {
    ...
    Blockly.Extensions.registerMutator(blockData.mutator as string, {
            saveExtraState: function(thist: any) {
                ...
                for (const inp of blockData.mutatorData?.inputs) {
                    if(this[inp.inputName ? inp.inputName : inp.text]) {
                        state[inp.inputName ? inp.inputName : inp.text] = this[inp.inputName ? inp.inputName : inp.text]
                    }
                }
                return state
            },
            loadExtraState: function(state: any) {
                for (const inp of blockData.mutatorData?.inputs) {
                    this[inp.inputName ? inp.inputName : inp.text] = state[inp.inputName ? inp.inputName : inp.text]
                }
                this.updateShape_()
            },
            compose: function(topBlock: any) {
            ...
            reconnectChildBlocks_: function (connections: Blockly.Connection/*needs arguments with connections*/) {},
...
export function CheckMutatorType(blockData: BlockDefinition, id: string) {
    ...
    switch (blockData.mutatorData?.type) {
        case MutatorType.CheckBox:
            if (!Blockly.Extensions.isRegistered(blockData.mutator as string)) CheckBoxMutator(blockData, id)
